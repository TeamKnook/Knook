...
assert.match(rules, /'phoneHash' in resource\.data/);
assert.match(rules, /!\('phoneHash' in resource\.data\)[\s\S]*request\.resource\.data\.phoneHash is string/);
assert.match(rules, /match \/matches\/\{matchId\}/);
assert.match(rules, /allow read: if signedIn\(\)[\s\S]*resource\.data\.status == 'active'[\s\S]*request\.auth\.uid in resource\.data\.participants/);
assert.doesNotMatch(rules, /match \/matches\/\{matchId\} \{\s*allow read: if activeParticipant\(matchId\)/);
