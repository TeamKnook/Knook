...
async def request_otp(body: RequestOtpIn):
    ...
    # In real Firebase this code never leaves Google. For preview we return it.
    code = "123456"
...
async def verify_otp(body: VerifyOtpIn):
    ...
    # Preview-mode: accept the dev code OR any 6-digit code
    if body.code != record["code"] and not re.fullmatch(r"\d{6}", body.code or ""):
    ...
    return AuthOut(token=uid, uid=uid, onboardingCompleted=False)
...
async def trigger_reveal():
    """Mirror of the dailyReveal Cloud Function. Exposed so the preview can
    demo the 6:30 PM IST reveal moment without waiting."""
    revealed_at = now()
